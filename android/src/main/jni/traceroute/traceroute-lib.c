...
void call_java_append_result(const char *text) {
  ...
  jmethodID j_append_result = (*jniEnv)->GetMethodID(
      jniEnv, j_cls, "appendResult", "(Ljava/lang/String;)V");
  if (j_append_result == NULL) {
    LOGE("can't find appendResult() method. do you add proguard to "
         "proguard-rules.pro");
    return;
  }
...
void *do_execute(void *args) {
  ...
  JNIEnv *jniEnv;
  jint status = (*g_jvm)->AttachCurrentThread(g_jvm, &jniEnv, NULL);
  if (status != 0) {
  ...
  for (i = 0; i < size; i++) {
    jstring command =
        (jstring)((*jniEnv)->GetObjectArrayElement(jniEnv, j_argvs, i));
