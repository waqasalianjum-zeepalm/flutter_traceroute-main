...
/* Our local copy / shim for some ICMPv6 defs if system headers are lacking */
#include "libsupp/icmp6.h"

/* CLI helpers and versioning */
#include "include/version.h"
#include "libsupp/clif.h"
#include "traceroute.h"

/* Threads (needed for pthread_self and friends) */
#include <pthread.h>

/*
 * Linux errqueue support:
 * - On desktop Linux, prefer the real kernel header.
 * - On Android (bionic) or systems without it, provide a minimal shim with just
 *   the fields and macros we actually use.
 */
#if defined(__has_include)
#if __has_include(<linux/errqueue.h>)
#include <linux/errqueue.h>
#define TR_HAS_LINUX_ERRQUEUE 1
#endif
#endif

#ifndef TR_HAS_LINUX_ERRQUEUE
/* ---- Minimal errqueue shim (Android, etc.) ---- */
#include <stdint.h>
...
/* Origins used by this code path */
...
#endif
/* Offender address immediately follows the extended error struct in cmsg */
#ifndef SO_EE_OFFENDER
#define SO_EE_OFFENDER(ee) ((struct sockaddr *)((ee) + 1))
#endif
#endif /* !TR_HAS_LINUX_ERRQUEUE */

/* ---- portability helpers (keep your existing ones) ---- */
...
static CLIF_option option_list[] = {
    ...
    {"d", "debug", 0, "Enable socket level debugging", CLIF_set_flag, &debug, 0,
     0},
    {"F", "dont-fragment", 0, "Do not fragment packets", CLIF_set_flag,
     &dontfrag, 0, CLIF_ABBREV},
    {"f", "first", "first_ttl", "Start from the %s hop (instead from 1)",
     CLIF_set_uint, &first_hop, 0, 0},
    {"g", "gateway", "gate",
     "Route packets through the specified gateway "
     "(maximum " _TEXT(MAX_GATEWAYS_4) " for IPv4 and " _TEXT(
         MAX_GATEWAYS_6) " for IPv6)",
     add_gateway, 0, 0, CLIF_SEVERAL},
    {"I", "icmp", 0, "Use ICMP ECHO for tracerouting", set_module, "icmp", 0,
     0},
    {"T", "tcp", 0,
    ...
    {0, "UL", 0,
     "Use UDPLITE for tracerouting (default dest port is " _TEXT(
         DEF_UDP_PORT) ")",
...
int exec(int argc, char *argv[]) {
  ...
  if (wait_secs < 0 || here_factor < 0 || near_factor < 0)
    ex_error("bad wait specifications `%g,%g,%g' used", wait_secs, here_factor,
             near_factor);
...
static void print_probe(probe *pb) {
  ...
  const char *straddr = pb->res.sa.sa_family ? addr2str(&pb->res) : "*";
  printf("#progress, %u, %d, %s, %.3f", pthread_self(), ttl, straddr,
         pb->recv_time ? 1000 * (pb->recv_time - pb->send_time) : 0);
...
static void poll_callback(int fd, int revents) { ops->recv_probe(fd, revents); }
...
void tune_socket(int sk) {
  ...
  if (af == AF_INET) {
    ...
    if (flow_label) {
      ...
      if (setsockopt(sk, IPPROTO_IPV6, IPV6_FLOWLABEL_MGR, &flr, sizeof(flr)) <
          0)
